...
#subset into group A
P1.flow.data <- P1.flow.data[1:326]
...
for(i in seq_along(all.nodes)){
  ...
  pars <- colnames(flow.i)[c(8, 11, 14, 17, 20, 23, 26, 29)]
  P1.warping <- warpSet(flow.i, stains=pars)
...
# ggcyto(P1.CD4.gs[1:6], aes(x=`B3-A`, y=`B1-A`), subset="root") +
#   geom_hex(bins=128) +
...
# single cells, ignore root
# run for CD4 and CD8 GatingSet objects separately

######################
## CD4 nodes
######################
cd4.nodes <- getNodes(P1.CD4.gs)[2:length(getNodes(P1.CD4.gs))]

for(i in seq_along(cd4.nodes)){
  node <- cd4.nodes[i]
  flow.i <- getData(P1.CD4.gs, y=node)
...
## CD8 nodes
######################
cd8.nodes <- getNodes(P1.CD8.gs)[2:length(getNodes(P1.CD8.gs))]

for(i in seq_along(cd8.nodes)){
  node <- cd8.nodes[i]
  flow.i <- getData(P1.CD8.gs, y=node)
  ...
  for(x in seq_along(samp.names)){
    ...
    if(nrow(samp.mat) > 1){
      samp.mat <- samp.mat[, c("FSC-A", "SSC-A", "FSC-H", pars)]
      cell.prefix <- paste(samp, gsub(paste0("CD8", node), pattern="/", replacement="_"), sep=".")
      cell.names <- paste(cell.prefix, paste0("Cell", 1:nrow(samp.mat)), sep=".")
      rownames(samp.mat) <- cell.names
      node.list[[samp]] <- samp.mat
    } else if(nrow(samp.mat) < 1 & nrow(samp.mat) > 0){
  ...
  samp.list[[paste0("/CD8", node)]] <- do.call(rbind.data.frame,
                                               node.list)
}
...
write.table(all.cell.df,
            file="/mnt/scratcha/jmlab/morgan02/FACS/Milieu_Interieur/summary_files/Panel1A_summary.txt",
